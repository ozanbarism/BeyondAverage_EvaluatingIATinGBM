...
for sensor_count, houses in all_houses_classified.items():
    for house_id, house_df in houses.items():
        ...
        # Define the columns to check for continuity
        current_remote_sensor_cols = ['RemoteSensor{}_Temperature'.format(i) for i in range(1, sensor_count + 1)]
        relevant_cols = ['Indoor_AverageTemperature', 'Thermostat_Temperature', 'Outdoor_Temperature', 'CoolingRunTime', 'time'] + current_remote_sensor_cols
        
        # Filter rows with NaN in the relevant columns
        ...
        start_point = house_df['time'].iloc[0]
        end_point = start_point + pd.Timedelta(days=16)
        
        while end_point <= house_df['time'].iloc[-1]:
            subset = house_df[(house_df['time'] >= start_point) & (house_df['time'] < end_point)]
            
            if len(subset) == 32*24:  # 32 days * 24 hours/day
                all_houses_reduced[sensor_count][house_id] = subset
                break  # Once we've found a continuous subset for a house, we don't need to continue for that house
            
            # If this range isn't continuous, shift the window by one day
            start_point += pd.Timedelta(days=1)
            end_point += pd.Timedelta(days=1)
...
#%% THIS DATA IS ALSO AVAILABLE IN THE GIVEN LINK.
